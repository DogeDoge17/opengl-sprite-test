...
framebuffer drawBuffer = {0, 0, DEFAULT_DRAW_WIDTH, DEFAULT_DRAW_HEIGHT};
float GlobalScale = 1;
...
static texture *loadTextures(const char **paths, const size_t pathsc) {
  ...
  for (size_t i = 0; i < pathsc; ++i) {
    ...
    int w, h, n;
    unsigned char *data = stbi_load(full_path, &w, &h, &n, 4);
    if (!data) {
...
GLuint loadShaderDir(const char *source, GLenum type) {
  ...
  if (!success) {
    ...
    if (infoLog) {
      ...
      fprintf(
          stderr,
          "Shader compilation failed, but error log could not be retrieved\n");
    }
...
void setupQuad() {
  const float vertices[] = {
      ...
      1.0f, 1.0f, 0.0f, 0,     0,    1,     1.0f,  1.0f, -1.0f, 1.0f, 0.0f,
      0,    0,    1,    0.0f,  1.0f, -1.0f, -1.0f, 0.0f, 0,     0,    1,
      0.0f, 0.0f, 1.0f, -1.0f, 0.0f, 0,     0,     1,    1.0f,  0.0f,
  };
...
void createTransformationMatrix(float *matrix, const float translateX,
                                const float translateY, float scaleX,
                                float scaleY, const float rotation) {
...
void createOrthographicMatrix(float *matrix, const float left,
...
void changeShader(const GLuint *shader, const size_t choose, float width,
                  float height) {
  float projMatrix[16];
  ...
  if (choose == 5) {
    GLint sharpnessLoc =
        glGetUniformLocation(shader[choose], "u_SharpnessAmount");
    if (sharpnessLoc != -1) {
      glUniform1f(sharpnessLoc, 0.5f);
    }
  } else if (choose == 2 || choose == 7) {
    const GLint scaleFactorLoc =
        glGetUniformLocation(shader[choose], "u_ScaleFactor");
    if (scaleFactorLoc != -1) {
      glUniform1f(scaleFactorLoc, 4);
    }
    const GLint lanczosALoc =
        glGetUniformLocation(shader[choose], "u_LanczosA");
    if (lanczosALoc != -1) {
...
void calculateViewportWithAspectRatio(const int windowWidth,
                                      const int windowHeight,
                                      const int targetWidth,
                                      const int targetHeight, int *viewportX,
...
void createFBOs(framebuffer *normalFBO, framebuffer *msaaFBO, const int width,
                const int height) {
  ...
  glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGBA8,
                          drawBuffer.renderWidth, drawBuffer.renderHeight,
                          GL_TRUE);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                         GL_TEXTURE_2D_MULTISAMPLE, msaaFBO->colorTexture, 0);
...
int main(const int argc, char **argv) {
  ...
  SDL_Window *win = SDL_CreateWindow(title, WINDOW_WIDTH, WINDOW_HEIGHT,
  ...
  createFBOs(&drawBuffer, &msaaFBO, drawBuffer.renderWidth,
             drawBuffer.renderHeight);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, drawBuffer.renderWidth,
  drawBuffer.renderHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
  drawBuffer.colorTexture, 0); CHECK_GL_ERRORS(); if
  (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
      fprintf(stderr, "Framebuffer is not complete: %d\n",
  glCheckFramebufferStatus(GL_FRAMEBUFFER)); glDeleteFramebuffers(1,
  &drawBuffer.bufferId); SDL_DestroyWindow(win); SDL_Quit();
  }
  ...
  glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, msaaFBO.colorTexture);
  glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGBA8,
  drawBuffer.renderWidth, drawBuffer.renderHeight, GL_TRUE);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
  GL_TEXTURE_2D_MULTISAMPLE, msaaFBO.colorTexture, 0);
  ...
  GLuint shaders[] = {
      makeShaderProgram(loadShaderDir(simple_frag_shader, GL_FRAGMENT_SHADER),
                        vertexShader),
      makeShaderProgram(loadShaderDir(bicubic_frag_shader, GL_FRAGMENT_SHADER),
                        vertexShader),
      makeShaderProgram(loadShaderDir(lanczos_frag_shader, GL_FRAGMENT_SHADER),
                        vertexShader),
      makeShaderProgram(loadShaderDir(mitchell_frag_shader, GL_FRAGMENT_SHADER),
                        vertexShader),
      makeShaderProgram(
          loadShaderDir(catmull_rom_frag_shader, GL_FRAGMENT_SHADER),
          vertexShader),
      makeShaderProgram(
          loadShaderDir(adaptive_sharpen_frag_shader, GL_FRAGMENT_SHADER),
          vertexShader),
      makeShaderProgram(loadShaderDir(fsr_like_frag_shader, GL_FRAGMENT_SHADER),
                        vertexShader),
      makeShaderProgram(
          loadShaderDir(sharp_lanczos_frag_shader, GL_FRAGMENT_SHADER),
          vertexShader),
  };
  size_t shaderUse = 0;
  ...
  changeShader(shaders, shaderUse = 0, (float)drawBuffer.renderWidth,
               (float)drawBuffer.renderHeight);
  glBindVertexArray(quadVAO);
  CHECK_GL_ERRORS();
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&ev)) {
      if (ev.type == SDL_EVENT_QUIT) {
        ...
        glViewport(0, 0, WINDOW_WIDTH = ev.window.data1,
    ...
    glBindFramebuffer(GL_FRAMEBUFFER,
                      msaaEnabled ? msaaFBO.bufferId : drawBuffer.bufferId);
    glViewport(0, 0, drawBuffer.renderWidth, drawBuffer.renderHeight);
    ...
    for (int j = 0; j < SPRITE_COUNT; ++j) {
      ...
      if (!freezeSprites) {
        sprites[i].x += ((rand() % 2 == 0 ? 1 : -1)) *
                        ((rand() % drawBuffer.renderWidth) / 5000.0f - 0.01f) *
                        (float)(deltaTime * 60.0f);
        if (sprites[i].x > drawBuffer.renderWidth)
          sprites[i].x = 0;
        if (sprites[i].x < 0)
          sprites[i].x = drawBuffer.renderWidth;

        sprites[i].y += ((rand() % 2 == 0 ? 1 : -1)) *
                        ((rand() % drawBuffer.renderHeight) / 5000.0f - 0.01f) *
                        (float)(deltaTime * 60.0f);
        if (sprites[i].y > drawBuffer.renderHeight)
          sprites[i].y = 0;
        if (sprites[i].y < 0)
          sprites[i].y = drawBuffer.renderHeight;
        sprites[i].rot +=
            ((rand() % 100) / 500.0f - 0.1f) * (float)(deltaTime * 30.0f);
      }
      float modelMatrix[16];
      createTransformationMatrix(
          modelMatrix, sprites[i].x * GlobalScale, sprites[i].y * GlobalScale,
          sprites[i].texture.width * sprites[i].scale * GlobalScale,
          -sprites[i].texture.height * sprites[i].scale * GlobalScale,
          sprites[i].rot);
    ...
    if (msaaEnabled) {
      ...
      glBlitFramebuffer(0, 0, drawBuffer.renderWidth, drawBuffer.renderHeight,
    ...
    calculateViewportWithAspectRatio(
        WINDOW_WIDTH, WINDOW_HEIGHT, drawBuffer.renderWidth,
        drawBuffer.renderHeight, &viewportX, &viewportY, &viewportWidth,
        &viewportHeight);
    ...
    float modelMatrix[16];
    createTransformationMatrix(modelMatrix, 0, 0, viewportWidth, viewportHeight,
                               0);
    const GLint modelLoc = glGetUniformLocation(shaders[shaderUse], "model");
    if (modelLoc != -1)
      glUniformMatrix4fv(modelLoc, 1, GL_FALSE, modelMatrix);
    ...
    if (shaderUse != 0) {
      const GLint texSizeLoc =
          glGetUniformLocation(shaders[shaderUse], "u_TextureSize");
      if (texSizeLoc != -1) {
